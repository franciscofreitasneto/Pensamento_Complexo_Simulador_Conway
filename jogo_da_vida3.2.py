...
class Predador:
    def __init__(self, x, y):
        self.x = x
        self.y = y
        self.vida = 200 # Maior tempo de vida para compensar a dificuldade de caça
        self.refeicoes = 3 # Energia acumulada necessária para reprodução
...
def atualizar_grade_numpy(grade):
    # Processamento rápido das presas usando a regra de Conway clássica
...
def decompor_predador(grade, x, y):
    """Quando o predador morre, há uma chance moderada de adubar o terreno."""
    # Reduzimos drasticamente o ruído entrópico: apenas 40% de chance de gerar vida
    if random.random() < 1:
        vazios = []
        for i in [-1, 0, 1]:
            for j in [-1, 0, 1]:
        ...
        if vazios:
            ...
            # Nasce apenas 1 presa para não sufocar a grade com ruído aleatório
            nx, ny = vazios[0]
            grade[ny, nx] = 1
...
def main():
    ...
    pygame.display.set_caption("* Simulador Eco-Organizacional - Homeostase e Complexidade *")
    relogio = pygame.time.Clock()

    grade_presas = criar_grade_aleatoria()
    # Começamos com uma população menor para dar tempo de a ordem se estabelecer
    predadores = inicializar_predadores(250) 
        ...
        if not pausado:
            ...
            for p in predadores_sobreviventes:
                if nova_grade_presas[p.y, p.x] == 1:
                    ...
                    p.vida = 15 # Recupera as energias
                    p.refeicoes += 1 # Acumula energia metabólica
                    
                    # Restrição: Só se reproduz após 3 refeições
                    if p.refeicoes >= 3:
                        p.refeicoes = 0 # Gasta a energia na reprodução
