...
def criar_grade_aleatoria():
    grade_aleatoria = np.zeros((LINHAS, COLUNAS), dtype=int)
    grade_quadrante = np.random.choice(
        [0, 1], 
        size=(LINHAS//2, COLUNAS//2), 
        p=[0.7, 0.3] # Altere o 0.2 para aumentar ou diminuir a desordem inicial
    )

    grade_aleatoria[LINHAS//(2*2):LINHAS//(2*2)+LINHAS//2,
            COLUNAS//(2*2):COLUNAS//(2*2)+COLUNAS//2] = grade_quadrante
